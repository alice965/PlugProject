...
총 <b>${cnt }</b> 개의 부스가 등록되어 있습니다.<hr>
	...
	<c:forEach var="obj" items="${list }">	
		<div style="background-color: black; float:left; width:256px; padding:3px; margin:5px; border-radius: 10px; ">
							 	...
							 	<c:forEach var="inst" items="${interest }">
								 	<!-- 부스번호와 내 관심부스번호가 같다면 (관심부스라면) -->
								 		<c:if test="${obj.NUM eq inst.NUM}">
								 			<i class="fa fa-star fa-lg bookmarked" aria-hidden="true" style="font-size:17px; color:white;"></i>
  ...
  <!------------------ 관심 부스 탭 시작--------------------------------------- -->
    <div id="bookmark"" class="tab-pane fade">
    	총 <b>${cntint }</b> 개의 관심 부스가 등록되어 있습니다.<hr>
		<!--부스 -->
...
$(".bookmark").click(function(){
	...
	if (r == true) {
		location.href = '/booth/addInterest?bthnum='
				+ $(this).next().val()
	} else {
		window.close();
	}
...
$(".bookmarked").click(function(){
	...
	if (r == true) {
		location.href = '/booth/deleteInterest?no='
				+ $(this).next().val()
